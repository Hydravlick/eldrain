...
combos_ref: "[[04_Player_Entities/Registries/Registry_Combos]]"
topology_ref: "[[04_Player_Entities/Two_Paradox_Vector_Matrix]]"
---

> Это необязательное интерактивное представление для Obsidian. Канонические данные лежат в [[04_Player_Entities/Registries/Registry_Combos|реестре Combo]]; его статическая карта остаётся читаемой без Dataview.
...
const combosPage = dv.page(dv.current().combos_ref);
...
function loadEntityFolder(folder, type) {
    const records = Array.from(dv.pages())
        .filter(page => page.type === "entity" && page.entity_kind === type && page.status === "active")
        .map(entityRecord)
