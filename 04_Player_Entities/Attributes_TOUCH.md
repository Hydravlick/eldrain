...
| `entropy_buffer` | ограниченный телесный вклад в фазовый удар | points | `consumer` | Gate Check `Shell.Entropy_Buffer` |

`gate_resist` поглощён `entropy_buffer` и больше не является отдельным substat. Средовая защита модулей считается отдельно и не заменяется телом.
