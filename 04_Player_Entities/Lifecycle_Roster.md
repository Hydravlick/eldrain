...
| `chronicle` | личные записи, пережитые события, traits, связи, шрамы и незавершённые цели | Access Price, стоимость тела или право печатать снаряжение |
...
Карточка Пешки показывает стабильные `module_capacity` её authored hero-kit `Race × Spec`. Chronicle-трейты не меняют эти ёмкости; выбранный [[07_Gear_Inventory/Thermos_System|Термос]] остаётся сменным предметом с собственными слотами и посадкой.
...
3.  **Восстановление:** В безопасной зоне раскрываются личность, hero-kit, биография и, для спасённого Foundling, один обычный `Origin trait` Chronicle.
...
* **Foundling (Найдёныш):** спасённый житель, древний выживший или человек с выраженным прошлым. Его ценность создают сочетание `hero_kit`, уже прожитая история, Chronicle, связи и будущие ситуации, а не отдельная человеческая категория.

Любая Ready-Пешка может быть выбрана для обычного shared-рейда. Доступ к конкретному сектору определяет вылазка и её реальные проверки, а не происхождение, статус Ward, наличие Chronicle или источник loadout. Базовая Пешка способна победить, эвакуироваться и развиться в пределах своей жизни. Кантрипная сила не растёт от близости смерти, а не закреплённый батарейным устройством эффект прекращается вместе с дееспособностью пользователя.

### Коридор стоимости Пешки

Пешка хранит личность и Chronicle, но не присваивает себе весь билд аккаунта. Hero-kit, доступные Frame, рецепты, схрон, городские сервисы и модульный каталог переживают KIA; у всех людей один Chronicle-ceiling, а его записи не меняют gunfeel, P/Q/E, арсенал или module capacity. Foundling получает один Origin trait раньше Ward, но не более высокий потолок.
...
Breakline — аварийный личный выход живой Пешки из уже проигрываемой вылазки. Он сохраняет человека и его Chronicle ценой всей текущей ставки, но не отменяет KIA и не является обязательным account gate.
...
BREAKLINE
  -> blind seam reached: Person + Chronicle return
  -> pursuit wins: KIA
```
...
- успешный Breakline возвращает ту же личность и Chronicle, но сам по себе не гарантирует trait, контрактную награду или экономический результат;
