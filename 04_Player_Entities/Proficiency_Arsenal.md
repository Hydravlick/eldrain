...
> Арсенал и обслуживание модулей принадлежат authored hero-kit. Личный тег конкретной Пешки не меняет базовую отдачу, точность, анимации, допустимый Frame или ёмкость модулей.
...
Ни Chronicle, ни Origin tag, ни случайная травма не обходят физический запрет и не добавляют оружие в арсенал. Новый хват, протез или носимый станок является отдельным видимым объектом с собственной ценой и уязвимостью, а не скрытым `+1`.
...
## 3. Личное mastery без нового proficiency

`prof` остаётся стабильным свойством hero-kit. Он не растёт от рейдов, не является качеством человека и не открывает дерево перков.

Поле `mastery_unlock` Frame публикует **кандидаты** на личный `tag_kind:: mastery`. Такой тег возникает только после конкретной практики, реликтовой процедуры или события и занимает одно из четырёх Major Tags Пешки.

Mastery-tag:

- не добавляет Frame и не повышает `prof`;
- не меняет базовые урон, отдачу, прицеливание, перезарядку или темп;
- открывает одну named technique внутри уже допустимой фазы Commitment;
- публикует отдельные tell, Exposure, цену и Recovery;
- не может быть вторым тегом, меняющим ту же фазу действия.

Таким образом, две одинаково экипированные Пешки одного hero-kit имеют одинаковый базовый moveset и gunfeel. Одна из них может совершить заранее читаемый личный приём, но не стреляет «чуть иначе» скрытым коэффициентом.
...
- два экземпляра одного hero-kit с одинаковым loadout различаются в базовом gunfeel из-за случайной личности; единственное допустимое исключение — named mastery technique с публичным tell и отдельной ценой.
