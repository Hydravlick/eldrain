...
>
> Практика / специализация задаёт методологию, а не готовые `Q/E` и не обязательную партийную роль. `Race × Practice` образует архетип, а экипировка, теги и условия операции создают итоговый профиль вылазки. Все игровые `P/Q/E` принадлежат конкретной ячейке `Race × Spec`.

`Spec` и исторические ID `assault`, `support`, `scout` остаются стабильными ключами схемы. Они не являются игроко-видимыми названиями и не описывают партийные роли.
