...
(async () => {
    ...
    dv.paragraph("Профиль больше не хранится в Registry_Combos. Он вычисляется из Race + Spec: раса дает первый вектор, класс дает второй, общая слабость берется из пересечения weak_to.");
