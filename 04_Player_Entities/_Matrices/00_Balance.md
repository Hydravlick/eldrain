...
(async () => {
    ...
    dv.paragraph("Профиль вычисляется из Race + Spec: раса дает первый вектор, класс дает второй, общая слабость берется из пересечения weak_to. MVP использует Ёж/Крыса/Белка × Авангард/Технократ/Странник. Все девять слотов существуют в Registry_Combos, но готовность контента определяется design_status и двойной проверкой контракта 3×3.");
