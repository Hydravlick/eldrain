...
(async () => {
    ...
    dv.paragraph("Профиль вычисляется из Race + Spec: раса дает первый вектор, класс дает второй, общая слабость берется из пересечения weak_to. Игровой MVP должен выбрать 3 расы и 3 класса и закрыть все 9 пар. Полная 5x5 остается внутренней картой расширения; текущие 9 curated-комбо еще не являются завершенной MVP-матрицей 3x3.");
