...
(async () => {
    ...
    const archetypes = comboResult.records.map(combo => {
        ...
        return {
            ...
            issues: [
                ...
                spec ? null : `Не найдена специализация: ${combo.reqSpec}`,
    ...
    dv.paragraph("Архетип вычисляется из Race + Spec: раса дает первый вектор, специализация дает второй, общая слабость берется из пересечения weak_to. Фактический профиль вылазки возникает из полной сборки и условий операции. MVP использует Ёж/Крыса/Белка × Авангард/Технократ/Странник. Все девять слотов существуют в Registry_Combos, но готовность контента определяется design_status и двойной проверкой контракта 3×3.");
