...
| R43 | Крафт распадается на RecipeTransaction и отдельную POE-подобную систему сфер | `RecipeTransaction` является единственным контрактом; Blueprint даёт носитель/право, Decorator направляет разрешённый исход, а полный reroll и лестница сфер удалены | fixed | `Barter_System`, `Blueprints`, `Craft_Modifiers`, `Registry_CraftingRecipes` |
