---
type: reference
status: active
system: reference_notes
tags: [reference, seeds, narrative, legacy_todo]
source:
---
# Reference Notes New

> Полезные narrative seeds из старого legacy TODO. Это не рабочая доска: actionable-пункты живут в [[09_Project_Management/TODO]].

## System References

- [[10_Reference/Reference Note Darkest Dungeon — командостроение, соло и повторяющиеся составы|Darkest Dungeon: командостроение, соло и повторяющиеся составы]] — reference для матрицы `Race × Spec`, свободных повторяющихся составов, соло-валидности и синергии через боевые окна.
- [[10_Reference/Reference Note Dark and Darker — вес способностей и темп боя|Dark and Darker: вес способностей и темп боя]] — reference для медленных ситуативных `P/Q/E`, обязательства, телеграфа, уязвимости и Recovery без превращения Элдрейна в ability-shooter.
- [[10_Reference/Reference Note Dark and Darker + Path of Exile — мусор, рецепты и торговый крафт|Dark and Darker + Path of Exile: мусор, рецепты и торговый крафт]] — reference для DaD-приятной петли торговцев, looted-мусора, affinity/рецептов и PoE-подобного аффиксного крафта через материалы-действия.
- [[10_Reference/Reference Note Анализ Mewgenics как эталона Single-Player Extraction Roguelike|Mewgenics: теги/трейты, наследие ростера и системные поломки]] — reference для единой модели `tag = trait`, биографического проявления свойств, долгих травм, rescue-наследия и мета-прогрессии ростера без буквального breeding.
- [[10_Reference/Reference Note Deadlock — способности как условия, экономика темпа и здоровый геройский бой|Deadlock: способности как условия]] — reference для геройских способностей, которые создают окна, двигают экономику темпа и не заменяют оружие/позицию.
- [[10_Reference/Reference Note Marathon — power floor, фракции и карты-механизмы|Marathon: power floor, фракции и карты-механизмы]] — reference для фракционной baseline-прогрессии, recovery-run без прогресса контрактов и POI как процедур доступа.
- [[10_Reference/Reference Note ARC Raiders — approachable extraction, map conditions и PvE как третья сила|ARC Raiders: map conditions и PvE]] — reference для видимых условий карты, approachable extraction, гаджетов, крафта и PvE как третьей силы.
- [[10_Reference/Reference Note Хроники Роркха — рейдовая Аномалия, безысходность и обучающий пермадез|Хроники Роркха: рейдовая Аномалия, безысходность и обучающий пермадез]] — reference для среды как источника давления, локально изучаемого ужаса, песочницы Сущности и смерти, которая обязана становиться знанием.
- [[10_Reference/Reference Note Competitor Mechanics Matrix — extraction, способности, соло и анти-кемпинг|Competitor Mechanics Matrix]] — сводная матрица выводов по DaD, Deadlock, Marathon, ARC Raiders и The Cycle для Gate Check, соло/сквадов, self-res и анти-кемпинга.
- [[10_Reference/Reference Note Техническая архитектура способностей и vertical slice|Техническая архитектура способностей и vertical slice]] — отделяет проверку игровых правил от выбора GAS, машины состояний, data-driven слоя и объёма производственного среза.

## The Keeper's Breach

**Отклонённое основание seed (Batch 9B):** прежняя версия запускала сцену после высокоуровневой эвакуации и трактовала игрока как выращивающего эффективные тела. Она не является действующим условием Recognition или terminal offer и не утверждает вкладку наследия либо награду.

Для будущей сцены сохраняется сдержанный профессиональный тон. Действующая граница [[04_Player_Entities/Entity_Grimoire#Позднее раскрытие|Recognition]] отделяет распознавание Осколка от [[04_Player_Entities/Life_Closure#Keeper extension point|отдельного предложения конкретной Пешке]]. Само предложение пока не определено.

## Selection Protocol

**Историческая, отклонённая предпосылка:** старая версия Архивариуса Прайма представляла Осколок инструментом выращивания пригодных тел для Якоря. Batch 9B не принимает эту предпосылку. Её нельзя использовать как reward contract, оценку стоимости Пешки или основание Recognition. Возможная тональность старого текста не канонизирует его механику; актуальную границу задаёт [[04_Player_Entities/Life_Closure#Keeper extension point|Life Closure]].

## Blood Debt Tutorial

Игрок случайно уничтожает стабилизированный ценный товар в Порту и получает долг перед местной силой.

Цепочка может обучать:

- лутингу: добыть батареи или стабилизаторы;
- стелсу: украсть инструменты без тревоги;
- экстракции: вынести кейс или груз через выход.

**Функция:** игрок получает личную причину идти в рейды до того, как игра просит его спасать мир.

## Stabilized Living Cargo

В хабе или Порту могут стоять клетки с тяжелыми стабилизированными существами.

Они показывают, что аномальные тела после стабилизации становятся товаром: еда, рабочая сила, сигнализация, редкий материал. Идея годится для атмосферы первых контрактов Круга Первого Приёма, Весовых Домов, Домов Пробы и Ночных Поручителей.

## Redemption Runs

Поздняя мета-идея для найденышей с тяжелыми травмами или плохими тегами/трейтами.

Такой персонаж дешевле на рынке, но дает повышенную награду за выживание в сложном рейде. Это не MVP, но может стать режимом для хардкорных игроков и стримеров.

## Cosmogony Overhaul

Старый `Lore_Update` сохранял полезную тональность: Элдрейн не поле битвы добра и зла, а результат ошибки цивилизации, которая слишком рано получила доступ к фундаментальным технологиям.

Полезные зерна:

- **Древние:** не обязательно боги, а архитекторы сверхреальности и стандартов реальности.
- **Хранители:** не мудрые стражи, а испуганные наследники катастрофы, которые удерживают стоп-кран Якоря.
- **Истинные Матрицы:** не просто мощные батареи, а фрагменты стандарта реальности.
- **Маги как Living Circuits:** живые проводники, которые платят здоровьем и рассудком за работу без артефактного фильтра.

**Назначение:** использовать как reference для будущего прохода по [[02_World_Lore/The_Collapse]], [[02_World_Lore/The_Anchor]], [[02_World_Lore/Magipunk_Physics]] и [[05_Combat_Survival/Magic_Batteries]].
