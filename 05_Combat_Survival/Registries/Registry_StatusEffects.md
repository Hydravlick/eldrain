...
index_order: 10
index_summary: "Хранит схему и записи: Реестр: Статусные Эффекты."
read_when: "Когда нужен контракт «Реестр: Статусные Эффекты» и его границы с соседними владельцами."
---
...
См. [[05_Combat_Survival/Status_Effects#Бюджет статуса: не геройский шутер]].

## Граница глобального статуса

См. [[05_Combat_Survival/Status_Effects#Граница глобального статуса]].
...
Первое применение переводит Пешку из `Clear` в `Strained`. Допустимый повтор до лечения создаёт постоянный Scar tag из семейства проводника и переводит острое состояние в `Scarred` по [[05_Combat_Survival/Magic_Batteries#5. Кантрипы|Cantrip owner]]. Дальнейшая перегрузка создаёт `Broken`: связанная функция отказывает, Пешка теряет сознание либо возникает lethal event по контракту способности; его исход определяет [[04_Player_Entities/Lifecycle_Resolver|Lifecycle Resolver]]. Все кантрипы одной Пешки используют эту запись совместно. Обычная медицина снимает acute debt; permanent Scar и его отдельное лечение принадлежат [[04_Player_Entities/Tags_System#Scar и адресное лечение|Scar owner]], без универсального HP-loss.
...
См. [[05_Combat_Survival/Status_Effects#Среда не является таблицей реакций]].
