...
**Диссонанс** — механическое название реакции Аномалии на чужеродную материю и активные эфирные действия. Игрок читает её через фон предмета, вспышку, предупреждение и ответ мира. `DissonanceLoad` и `DissonancePulse` — внутренние метрики; название не устанавливает универсальную физику мира или обязательное слово жителей.
...
### Что даёт основание для реакции

Подтверждённые источники — материальная принадлежность текущей сессии, работа маготехнического контура, физически объяснённое изменение тела и конкретное эфирное occurrence. Звук, цена покупки, репутация и общий рейтинг силы не заменяют эти источники.

Предметный контракт [[07_Gear_Inventory/Dissonance_Value|Dissonance Value]] ещё содержит спорные зависимости: «стабилизированную ценность» и `Hot PvP`. Их коэффициенты сохранены, но физическое основание не установлено. Это `REVIEW: MECHANIC`, а не повод назвать экономическую метку резонансом. Корреляция редкости и цены с устройством предмета не является формулой начисления.
