...
Pattern — повторяемая физическая конструкция внутри Frame. Он определяет конкретный moveset: Primary, необязательный Alt, необязательную поддержку Weapon Focus, траектории, подготовку, порядок операций, сервисные требования, видимые состояния конструкции и поведение выпуска. Weapon Focus и Alt не обязательны и не являются одной операцией. Pattern не знает конкретного соседнего Pattern и не публикует pair-specific combo matrix.
