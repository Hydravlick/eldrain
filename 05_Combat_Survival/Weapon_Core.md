...
Frame не вводит собственный числовой атрибутный допуск. Он становится доступен через арсенал и телесную capability, раскрывает технику через proficiency, а затем может использовать уже существующий именованный consumer T.O.U.C.H.

| Атрибут | Допустимое влияние на конкретный Frame | Запрет |
|:---|:---|:---|
| `TRQ` | удержание тяжёлой нагрузки и осадной позиции | общий физический урон |
| `GRP` | взвод, замена батареи и возврат руки | aim assist или скрытая точность |
| `LYR` | переносимость собственной отдачи, ожога и Backlash | броня от входящего урона |
| `GLW` | охлаждение, перегруз и устойчивость энергетического цикла | общий эфирный resist |
| `SNS` | чтение weakspot, предупреждение перегрева и прогноз дрейфа | wallhack или чтение врага сквозь сцену |

Каждый Frame обязан назвать свой consumer и конечный параметр; строка таблицы не даёт все перечисленные эффекты автоматически.
