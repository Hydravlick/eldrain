...
* **Броня:** не добавляет ячейки быстрого доступа. Ось `cargo` помогает работать с тяжелым грузом и Back Slot, но не превращается в скрытый объем инвентаря. Модуль `battery_rack` — отдельный энергетический контракт: он увеличивает лишь заранее подготовленную очередь батарей назначенного контура, не список предметов.
