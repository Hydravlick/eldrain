...
index_order: 200
index_summary: "Определяет состояния, разрешение и связи: Контракт калибровки комплектов."
read_when: Когда нужен контракт «Контракт калибровки комплектов» и его границы с соседними владельцами.
---
# Контракт калибровки комплектов

## 1. Цель

Этот контракт задаёт состав проверяемых комплектов и вопросы калибровки. Он не владеет значениями предметов, защитой, давлением или правилами входа.
...
## 2. Поля отчёта калибровки

Для каждого предмета, влияющего на вход в рейд или экономику, отчёт собирает применимые поля из его владельцев:
...
Расчёты выполняются по [[08_World_Generation/Generation/Gate_Check]], [[05_Combat_Survival/Dissonance_System]] и [[05_Combat_Survival/Threat_Thresholds]]. Эта проверка читает результаты владельцев; собственных формул защиты или давления у неё нет.
...
| Balanced | Reload и battery-powered Q/E конкурируют за Full Battery ItemIDs | два канала не закрывают все функции | переход сохраняет долг предыдущего действия | кантрип остаётся аварийным решением |
...
Для сравнения фиксировать расход отдельных Full Battery на reload и battery-powered Q/E, долю неразрешённых отмен Recovery, максимальную цепочку hard control и риск-скорректированную прибыль живого и жертвуемого Welfare.
