...
tags:
  - factions
type: view
view_kind: faction_relationships
upstream_sources:
  - "[[03_Factions_Societies/Registries/Registry_Factions]]"
  - "[[03_Factions_Societies/Lore/Civic_Order]]"
---

# Отношения городских сторон

Производная карта читает идентичность и направленные `rel_*` записи фракционных сущностей через [[03_Factions_Societies/Registries/Registry_Factions]]. Она показывает отношения и причины, но не определяет репутацию, услуги или доступ.
...
try {
    ...
    const sourceFolder = dv.current().file.path;
    ...
    const factionPages = Array.from(dv.pages())
        .filter(page => page.type === "entity" && page.status === "active" && page.entity_kind === "faction")
