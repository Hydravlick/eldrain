...
# Биомы: среда, враги и добыча

Производное сопоставление [[08_World_Generation/Registries/Registry_Biomes]], [[08_World_Generation/Registries/Registry_Mobs]] и [[07_Gear_Inventory/Registries/Registry_Items]]. Таблица не задаёт spawn policy и не обещает конкретную награду.
...
for (const biomeBlock of biomeBlocks) {
    ...
    if (!biomeId || biomeId.startsWith("template_")) continue;
    ...
    levels.forEach(lvlBlock => {
        ...
        const lvlMatch = lvlBlock.match(/\[difficulty::\s*(\d+)\]/i);
