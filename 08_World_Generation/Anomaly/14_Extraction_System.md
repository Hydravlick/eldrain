...
Обычные Пороги доступны только до lifecycle `Seal`. После Seal новые search/assignment/sync операции не создаются, а незавершённые операции завершаются согласно атомарному порядку [[08_World_Generation/Generation/07_Server_Lifecycle|Server Lifecycle]].

Sealed Apex `05:00–06:00` — другой контракт выживания. Dawn не является Порогом, бесплатным slot или поздним `SyncLease`.

## Handoffs

- [[08_World_Generation/Generation/20_Egress_Solvency|Egress Solvency]] доказывает, что joinable pre-Seal envelope покрывает обязательства; он не выбирает anchor и не решает Sync.
...
- [[08_World_Generation/Generation/07_Server_Lifecycle|Server Lifecycle]] задаёт Seal/Dawn order.
- [[08_World_Generation/Anomaly/17_Apex_Last_Hour|Apex Last Hour]] управляет sealed survival grammar, а не обычными выходами.
