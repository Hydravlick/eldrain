...
- **Frame / Pattern / ItemID / Action** — переносимый оружейный язык / повторяемая конструкция с moveset / физическая копия со ссылкой на Pattern / конкретное исполнение и принятый долг. Definition ID повторяемой конструкции — `pattern_id`; физические копии имеют независимые ItemID. [[05_Combat_Survival/Weapon_Core|Weapon Core]].
