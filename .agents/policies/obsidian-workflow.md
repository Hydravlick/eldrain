...
[Routable_Owners.base](../../09_Project_Management/Routable_Owners.base) lists active routable pages in gameplay domains, reading their properties directly. Verify native results with `obsidian vault=Eldrain base:query path=09_Project_Management/Routable_Owners.base format=json` and compare paths with route metadata. `index_route: owner` designates an authority entrypoint and is forbidden on `type: view`. A successful query verifies declared routing membership, not the semantic correctness of each ownership claim. The Base itself stays derived and stores no copied canonical data.
